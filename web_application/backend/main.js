...
mongo.connect((error, client) => {
  ...
  exp.locals.hospitals      = client.db(db_name).collection("trains");
  exp.locals.doctors        = client.db(db_name).collection("passengers");
  exp.locals.patients       = client.db(db_name).collection("tickets");
  exp.locals.cured_patients = client.db(db_name).collection("soldTickets");
